...
#define MAXVALUE_TEST_SEMAPHORE_NAME    ("test_maxvalue")
...
TEST(named, fair)
{
	...
	ret = mutexLock(shared.lock);
	TEST_ASSERT_EQUAL(EOK, ret);

	flags = shared.flags;

	ret = mutexUnlock(shared.lock);
	TEST_ASSERT_EQUAL(EOK, ret);
...
TEST(named, maxvalue)
{
	...
	sem = sem_open(MAXVALUE_TEST_SEMAPHORE_NAME, O_CREAT | O_EXCL, 0, 1);
	TEST_ASSERT_EQUAL(EOK, errno);
	TEST_ASSERT_NOT_EQUAL(SEM_FAILED, sem);

	ret = sem_post(sem);
	TEST_ASSERT_EQUAL(EOVERFLOW, errno);
	TEST_ASSERT_EQUAL(-1, ret);
	...
	ret = sem_unlink(MAXVALUE_TEST_SEMAPHORE_NAME);
	TEST_ASSERT_EQUAL(EOK, errno);
	TEST_ASSERT_EQUAL(EOK, ret);
}
