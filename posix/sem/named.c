...
static struct pthread_attr_t thread_attrs;
...
TEST_SETUP(named)
{
	...
	pthread_attr_setstacksize(&thread_attrs, 4096);
}
...
TEST(named, wait)
{
	...
	ret = pthread_create(&thread, &thread_attrs, wait_helper_thread_func, NULL);
	TEST_ASSERT_EQUAL(EOK, errno);
...
TEST(named, trywait)
{
	...
	ret = pthread_create(&thread, &thread_attrs, trywait_helper_thread_func, NULL);
	TEST_ASSERT_EQUAL(EOK, ret);
...
TEST(named, timedwait)
{
	...
	ret = pthread_create(&thread, &thread_attrs, timedwait_helper_thread_func, NULL);
	TEST_ASSERT_EQUAL(EOK, ret);
...
TEST(named, fair)
{
	...
	ret = pthread_create(&waiter1, &thread_attrs, fair_helper_waiter1_thread_func, NULL);
	TEST_ASSERT_EQUAL(EOK, ret);

	ret = pthread_create(&waiter2, &thread_attrs, fair_helper_waiter2_thread_func, NULL);
	TEST_ASSERT_EQUAL(EOK, ret);
