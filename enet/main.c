...
static void make_rand_frame(uint8_t *buf, size_t buf_sz)
{
...
static void set_iface_loopback(int fd, struct ifreq *ifr, bool enable)
{
	int err;
...
#define TEST_FAIL_FMT_MESSAGE(fmt, ...) \
	do { \
		char err_msg_buf[64]; \
		snprintf(err_msg_buf, 64, fmt, ##__VA_ARGS__); \
		TEST_FAIL_MESSAGE(err_msg_buf); \
	} while (0)


static int setup_socket(const struct sockaddr_ll *addr, const char *name)
{
	int s = socket(AF_PACKET, SOCK_RAW, htons(ETH_P_ALL));
	if (s < 0) {
		TEST_FAIL_FMT_MESSAGE("%s: socket failed: %s (%d)", name, strerror(errno), errno);
	}

	if (addr != NULL) {
		if (bind(s, (const struct sockaddr *)addr, sizeof(*addr)) < 0) {
			TEST_FAIL_FMT_MESSAGE("%s: bind failed: %s (%d)", name, strerror(errno), errno);
		}
	}
