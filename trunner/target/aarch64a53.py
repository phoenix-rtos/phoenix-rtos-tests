...
class AARCH64A53SomRebooter(Rebooter):
    """Rebooter for aarch64a53-zynqmp-som."""
...
class AARCH64A53SomSyspageLoader(PloRamSyspageLoader):
    """Loads app binaries and file blobs into RAM via JLink OpenOCD GDB server
    and registers them with PLO on aarch64a53-zynqmp-som."""
...
class AARCH64A53SomTarget(TargetBase):
    """Target for aarch64a53-zynqmp-som (Zynq UltraScale+ RPU R5 on SOM board)."""

    name = "aarch64a53-zynqmp-som"
    rootfs = False
    gdb_port = 3334
    ...
    def __init__(self, host: Host, port: Optional[str] = None, baudrate: int = 115200):
        ...
        self.rebooter = AARCH64A53SomRebooter(host, self.dut, self.openocd_config)
        super().__init__()
    ...
    def build_test(self, test: TestOptions) -> Callable[[TestResult], TestResult]:
        ...
        if test.bootloader is not None:
            ...
            if test.bootloader.apps or test.bootloader.files:
                syspage_loader = AARCH64A53SomSyspageLoader(
                    dut=self.dut,
...
class AARCH64A53Zcu104Rebooter(Rebooter):
    """Rebooter for aarch64a53-zynqmp-zcu104."""
    ...
    def _reboot_by_debugger(self):
        """Reset the board via SRST through OpenOCD FTDI."""
        args = [
            "openocd",
            "-c adapter driver ftdi",
            "-c ftdi vid_pid 0x0403 0x6011",
            "-c ftdi channel 0",
            "-c ftdi layout_init 0x00c8 0x000b",
            "-c ftdi layout_signal POR_RST -data 0x0040 -oe 0x0040",
            "-c ftdi layout_signal nSRST -data 0x0080 -oe 0x0080",
            "-c transport select jtag",
...
class AARCH64A53Zcu104SyspageLoader(PloRamSyspageLoader):
    """Loads app binaries and file blobs into RAM via FTDI OpenOCD GDB server
    and registers them with PLO on aarch64a53-zynqmp-zcu104."""
    ...
    @contextmanager
    def _openocd_gdb_server(self):
        """Starts OpenOCD as a GDB server with FTDI adapter, halting the target."""
...
class AARCH64A53Zcu104Target(TargetBase):
    """Target for aarch64a53-zynqmp-zcu104 (Zynq UltraScale+ RPU R5 on SOM board)."""

    name = "aarch64a53-zynqmp-zcu104"
    rootfs = False
    gdb_port = 3333
    openocd_adapter_speed = 12000
    openocd_config_file = "scripts/openocd/zynqmp/xilinx_zynqmp.cfg"
    ...
    def __init__(self, host: Host, port: Optional[str] = None, baudrate: int = 115200):
        ...
        self.dut = SerialDut(port, baudrate, encoding="utf-8", codec_errors="ignore")
        self.openocd_config = f"{self._project_dir()}/{self.openocd_config_file}"
        self.rebooter = AARCH64A53Zcu104Rebooter(host, self.dut, self.openocd_config)
        super().__init__()
    ...
    def flash_dut(self, host_log: TextIO):
        ...
        self._run_openocd_jlink(
            commands=[
                ...
                "-c halt",
                f"-c load_image {self.plo_ram_image} {self.plo_ram_addr:#x} bin",
                f"-c resume {self.plo_ram_addr:#x}",
                "-c exit",
            ],
            cwd=self.boot_dir(),
        )
    ...
    def build_test(self, test: TestOptions) -> Callable[[TestResult], TestResult]:
        ...
        if test.bootloader is not None:
            ...
            if test.bootloader.apps or test.bootloader.files:
                syspage_loader = AARCH64A53Zcu104SyspageLoader(
                    dut=self.dut,
                    apps=test.bootloader.apps,
                    gdb=GdbInteractive(
                        port=self.gdb_port,
