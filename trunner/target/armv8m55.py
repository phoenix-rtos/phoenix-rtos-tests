...
from trunner.tools import GdbInteractive, STM32ProgrammerCLIProcess, STM32ProgrammerCLIError, STLinkGdbServer
...
class ARMv8M55Rebooter(Rebooter):
    ...
    def _reboot_by_debugger(self):
        with self.gdb.run():
            self.gdb.proc.sendline("set architecture arm")
            self.gdb.expect_prompt()
            self.gdb.connect()
            self.gdb.proc.sendline("monitor reset")
            self.gdb.expect_prompt()
            self.gdb.cont()
            time.sleep(0.5)
