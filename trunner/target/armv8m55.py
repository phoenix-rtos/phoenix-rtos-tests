...
class STM32N6STLinkGdbServerHarness(IntermediateHarness):
    ...
    def __call__(self, result: TestResult) -> TestResult:
        with OpenocdGdbServer(interface="stlink", target="stm32n6x").run():
            self.harness(result)
...
class STM32N6Target(TargetBase):
    ...
    def flash_dut(self, host_log: TextIO):
        try:
            ...
            with OpenocdGdbServer(interface="stlink", target="stm32n6x", host_log=host_log).run():
        ...
        except (FileNotFoundError, OpenocdError) as e:
