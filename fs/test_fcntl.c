		...
		for (i = 0, failed = 0; i < test_len; i++) {
			...
			if ((result == -1 && output_vector[i]) || (result != -1 && !output_vector[i])) {
				...
				if (!verbose_test())
					continue;
				if (result == -1)
					printf("[%d] F_SETLK failed, but expected to pass - l_start %lld l_len %lld l_type %s\n",
						i, (long long)input_vector[i].l_start, (long long)input_vector[i].l_len, l_type_str[input_vector[i].l_type]);
				else
					printf("[%d] F_SETLK passed, but expected to fail - l_start %lld l_len %lld l_type %s\n",
						i, (long long)input_vector[i].l_start, (long long)input_vector[i].l_len, l_type_str[input_vector[i].l_type]);
			}
		}
		exit(failed);
...
int getlk_assert(int fd, struct flock *input_fl, struct flock *output_fl)
{
	...
	case 0:
		...
		if (result && verbose_test()){
			printf("\nOutput: l_type %s , l_whence %d, l_start %lld, l_len %lld, l_pid %d\n",
				l_type_str[input_fl->l_type], input_fl->l_whence, (long long)input_fl->l_start, (long long)input_fl->l_len, input_fl->l_pid);
			printf("Expected: l_type %s , l_whence %d, l_start %lld, l_len %lld, l_pid %d\n",
				l_type_str[output_fl->l_type], output_fl->l_whence, (long long)output_fl->l_start, (long long)output_fl->l_len, output_fl->l_pid);
		}
