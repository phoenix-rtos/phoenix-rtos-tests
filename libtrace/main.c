...
#define FILE_NOT_DIR_PATH (TRACES_DIR "/file")
...
TEST_SETUP(test_libtrace)
{
	struct stat st;
	errno = 0;
	if (stat(TRACES_DIR, &st) < 0 && errno != ENOENT) {
		FAIL("stat");
	}
	if (errno == ENOENT && mkdir(TRACES_DIR, 0777) < 0) {
		FAIL("mkdir");
	}
...
TEST(test_libtrace, test_libtrace_err)
{
	...
	TEST_ASSERT_EQUAL_INT(-EINVAL, trace_stopAndGather(&ctx, BUF_SIZE, TRACES_DIR "/trace"));
	TEST_ASSERT_EQUAL_INT(0, memcmp(&ctx, &og_ctx, sizeof(trace_ctx_t)));

	TEST_ASSERT_EQUAL_INT(-EINVAL, trace_record(&ctx, 100, 1000, 0, TRACES_DIR "/trace"));
	TEST_ASSERT_EQUAL_INT(0, memcmp(&ctx, &og_ctx, sizeof(trace_ctx_t)));

	TEST_ASSERT_EQUAL_INT(0, trace_start(&ctx));
	TEST_ASSERT_EQUAL_INT(-EINVAL, trace_stopAndGather(&ctx, 0, TRACES_DIR "/trace"));
	TEST_ASSERT_EQUAL_INT(0, memcmp(&ctx, &og_ctx, sizeof(trace_ctx_t)));
	 ...
	 */
	TEST_ASSERT_EQUAL_INT(0, trace_start(&ctx));
	TEST_ASSERT_EQUAL_INT(0, trace_stopAndGather(&ctx, BUF_SIZE, TRACES_DIR "/trace"));
	TEST_ASSERT_EQUAL_INT(0, memcmp(&ctx, &og_ctx, sizeof(trace_ctx_t)));
}
...
TEST(test_libtrace, test_libtrace_start_stop)
{
	...
	for (size_t rep = 0; rep < 3; rep++) {
		...
		TEST_ASSERT_EQUAL_INT(0, trace_stopAndGather(&ctx, BUF_SIZE, TRACES_DIR "/start_stop"));
		TEST_ASSERT_EQUAL_INT(0, memcmp(&ctx, &og_ctx, sizeof(trace_ctx_t)));
	}
}
...
TEST(test_libtrace, test_libtrace_record)
{
	...
	for (size_t rep = 0; rep < 3; rep++) {
		TEST_ASSERT_EQUAL_INT(0, trace_record(&ctx, 100, 100, BUF_SIZE, TRACES_DIR "/start_stop"));
		TEST_ASSERT_EQUAL_INT(0, memcmp(&ctx, &og_ctx, sizeof(trace_ctx_t)));
	}
}
