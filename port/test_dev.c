 ...
 * test_dev
 *
 * tests for `create_dev()` and `destroy_dev()` userspace function
 *
...
TEST(test_dev, complex_relpath)
{
	...
	/* should not be able to delete directories (even those created by create_dev */
	TEST_ASSERT_LESS_THAN(0, destroy_dev(DEV_PREFIX "/" COMPLEX_PATH));
