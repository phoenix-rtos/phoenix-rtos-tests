...
TEST(test_register, simple)
{
	...
	TEST_ASSERT_EQUAL_MEMORY(&oid, &ofil, sizeof(oid_t));
	TEST_ASSERT_EQUAL_MEMORY(&oid, &odev, sizeof(oid_t));
...
TEST(test_register, register_many)
{
	...
	for (int i = 0; i < NDEVS; i++) {
		...
		TEST_ASSERT_EQUAL_MEMORY_MESSAGE(&devs[i].oid, &ofil, sizeof(oid_t), devs[i].name);
		TEST_ASSERT_EQUAL_MEMORY_MESSAGE(&devs[i].oid, &odev, sizeof(oid_t), devs[i].name);
	}
