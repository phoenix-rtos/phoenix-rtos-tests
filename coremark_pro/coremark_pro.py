...
def harness(dut: Dut, ctx: TestContext, result: TestResult):
    ...
    while True:
        try:
            ...
            line_msg = "\n\t\t" + f"Error waiting for output: {type(e).__name__}"
            msg.append(line_msg)
            break
    ...
    if subresult is None:
        result.add_subresult(subname="prepare", status=Status.FAIL, msg="".join(msg))
        msg.clear()
    ...
    if subresult is None:
        result.add_subresult(subname=name, status=Status.FAIL, msg="".join(msg))
        return TestResult(status=Status.FAIL)
