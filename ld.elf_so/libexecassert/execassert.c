...
static int execAssert_start(const char *command, FILE **fout, FILE **ferr)
{
	int fdout[2];
	int fderr[2];
	int pid;
	...
	if (pid == 0) {
		TEST_ASSERT_GREATER_OR_EQUAL(0, dup2(fdout[1], STDOUT_FILENO));
		TEST_ASSERT_GREATER_OR_EQUAL(0, dup2(fderr[1], STDERR_FILENO));

		TEST_ASSERT_EQUAL(0, close(fdout[0]));
		TEST_ASSERT_EQUAL(0, close(fdout[1]));
		TEST_ASSERT_EQUAL(0, close(fderr[0]));
...
{
	FILE *fout, *ferr;
	int pid = execAssert_start(command, &fout, &ferr);
