...
static int get_ifconf(int fd, struct ifconf *ifc)
{
...
static int ethtool_ioctl(struct ifreq *ifr, void *ethtool_struct, uint32_t cmd, char *err_msg_buf)
{
...
{
	int res = get_ifconf(fd, &current_ifc);
...
TEST(test_ioctl_special, ifconf_not_enough_space)
{
	...
	int res = ioctl(fd, SIOCGIFCONF, &ifconf);
	TEST_ASSERT_EQUAL_MESSAGE(0, res, strerror(errno));
	TEST_ASSERT_EQUAL(sizeof(ifr), ifconf.ifc_len);

	/* ifr_name should be 3 characters in lwip.
	   if net stack is ever changed - this should change too */
	TEST_ASSERT_EQUAL(3, strnlen(ifconf.ifc_req->ifr_name, IFNAMSIZ));
}
...
TEST(test_ioctl_special, ethtool_gset)
{
	...
	int res = ethtool_ioctl(&current_ifr, &cmd, ETHTOOL_GSET, err_msg_buf);
	TEST_ASSERT_GREATER_OR_EQUAL_MESSAGE(0, res, err_msg_buf);
}
...
TEST(test_ioctl_special, ethtool_test)
{
	...
	if (ethtool_ioctl(&current_ifr, &cmd, ETHTOOL_TEST, err_msg_buf) < 0) {
		TEST_FAIL_MESSAGE(err_msg_buf);
	}
...
TEST(test_ioctl_special, ethtool_gloopback)
{
	...
	char err_msg_buf[ERR_MSG_LEN] = { 0 };
	if (ethtool_ioctl(&current_ifr, &cmd, ETHTOOL_GLOOPBACK, err_msg_buf) < 0) {
		TEST_FAIL_MESSAGE(err_msg_buf);
	}
}
...
TEST(test_ioctl_special, ethtool_sloopback)
{
	...
	do {
		...
		cmd.data = expected;
		if (ethtool_ioctl(&current_ifr, &cmd, ETHTOOL_SLOOPBACK, err_msg_buf) < 0) {
			break;
		}
		...
		if (err != EOPNOTSUPP && cmd.data != expected) {
