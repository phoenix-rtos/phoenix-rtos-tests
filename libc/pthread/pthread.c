...
static void *test_threadCleanupCancel(void *arg)
{
	...
	/* Ensure mutex is acquired */
	while (pthread_mutex_trylock(&thread_args.count_lock) == 0) {
		pthread_mutex_unlock(&thread_args.count_lock);
		sleep(0);
	}

	/* Avoid pop before pthread_cancel call */
	pthread_mutex_lock(&thread_args.count_lock);
...
TEST(test_pthread_cleanup, pthread_cleanup_push_pthread_cancel)
{
	...
	TEST_ASSERT_EQUAL_INT(0, pthread_join(thread, &retval));
	TEST_ASSERT_EQUAL_INT(PTHREAD_CANCELED, (intptr_t)retval);
