...
static ssize_t safe_pread(int fd, void *buf, size_t count, off_t offset)
{
	...
	do {
		ret = pread(fd, (char *)buf + bytes_read, count - bytes_read, offset + bytes_read);
		if (ret < 0) {
			if (errno == EINTR) {
				continue;
			}
...
{
	ssize_t ret, bytes_written = 0;
	do {
		ret = pwrite(fd, (const char *)buf + bytes_written, count - bytes_written, offset + bytes_written);
		if (ret < 0) {
			if (errno == EINTR) {
				continue;
			}
...
TEST_SETUP(unistd_pread)
{
	...
	TEST_ASSERT_EQUAL(LOREM_LEN, safe_write(fd, LOREM, LOREM_LEN));
...
TEST(unistd_pread, pread_offset)
{
	...
	TEST_ASSERT_GREATER_THAN_INT(0, fd);
	TEST_ASSERT_EQUAL(14, safe_pread(fd, buf, 14, 12));
	buf[14] = '\0';
	TEST_ASSERT_EQUAL_STRING("dolor sit amet", buf);
	TEST_ASSERT_EQUAL(11, safe_pread(fd, buf, 11, 6));
	buf[11] = '\0';
...
TEST(unistd_pread, pwrite_offset)
{
	...
	TEST_ASSERT_GREATER_THAN_INT(0, fd);
	TEST_ASSERT_EQUAL(9, safe_pwrite(fd, "OVERWRITE", 9, 12));
	TEST_ASSERT_EQUAL(26, safe_read(fd, buf, 26));
	buf[26] = '\0';
	TEST_ASSERT_EQUAL_STRING("Lorem ipsum OVERWRITE amet", buf);
	TEST_ASSERT_EQUAL(15, safe_pwrite(fd, "ipsum dolor sit", 15, 6));
	TEST_ASSERT_EQUAL(LOREM_LEN, safe_pread(fd, buf, LOREM_LEN, 0));
	buf[LOREM_LEN] = '\0';
...
TEST(unistd_pread, pread_multithread)
{
	...
	for (i = 0; i < 4; i++) {
		...
		TEST_ASSERT_EQUAL(15 * (i + 1) <= LOREM_LEN ? 15 : 11, args[i].ret);
	}
	buffer[LOREM_LEN] = '\0';
...
TEST(unistd_pread, pwrite_multithread)
{
	...
	TEST_ASSERT_EQUAL(LOREM_LEN, safe_pread(fd, read_buffer, LOREM_LEN, 0));
