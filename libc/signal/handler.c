...
TEST_TEAR_DOWN(handler)
{
	...
	for (int signo = 1; signo < USERSPACE_NSIG; ++signo) {
		/* SIGKILL/SIGSTOP dispositions can't be changed */
...
TEST(handler, unblock_pending_signal)
{
	...
	TEST_ASSERT_NOT_EQUAL(SIG_ERR, signal(SIGALRM, sighandler));
...
TEST_TEAR_DOWN(sigsuspend)
{
	...
	TEST_ASSERT_NOT_EQUAL(SIG_ERR, signal(SIGALRM, SIG_DFL));
...
TEST(sigsuspend, signal_before_handler)
{
	...
	errno = 0;
	TEST_ASSERT_EQUAL_INT(-1, sigsuspend(&all_unblocked));
	TEST_ASSERT_EQUAL_INT(EINTR, errno);
...
TEST(sigaction, unmask_changed_action_handler_to_ignore)
{
	...
	TEST_ASSERT_EQUAL_INT(0, sigemptyset(&masked));
	TEST_ASSERT_EQUAL_INT(0, sigaddset(&masked, SIGUSR1));

	TEST_ASSERT_EQUAL_INT(0, sigprocmask(SIG_SETMASK, &masked, NULL));
	TEST_ASSERT_NOT_EQUAL(SIG_ERR, signal(SIGUSR1, sighandler));
	TEST_ASSERT_EQUAL_INT(0, raise(SIGUSR1));
	TEST_ASSERT_NOT_EQUAL(SIG_ERR, signal(SIGUSR1, SIG_IGN));
	TEST_ASSERT_EQUAL_INT(0, sigprocmask(SIG_SETMASK, &empty, NULL));
	TEST_ASSERT_EQUAL_HEX32(0, handler_haveSignal);
	TEST_ASSERT_NOT_EQUAL(SIG_ERR, signal(SIGUSR1, sighandler));
	TEST_ASSERT_EQUAL_HEX32(0, handler_haveSignal);
}
...
TEST(sigaction, unmask_changed_action_handler_to_default_ignored)
{
	...
	TEST_ASSERT_EQUAL_INT(0, sigemptyset(&empty));
	TEST_ASSERT_EQUAL_INT(0, sigemptyset(&masked));
	TEST_ASSERT_EQUAL_INT(0, sigaddset(&masked, SIGURG));

	TEST_ASSERT_EQUAL_INT(0, sigprocmask(SIG_SETMASK, &masked, NULL));
	TEST_ASSERT_NOT_EQUAL(SIG_ERR, signal(SIGURG, sighandler));
	TEST_ASSERT_EQUAL_INT(0, raise(SIGURG));
	TEST_ASSERT_NOT_EQUAL(SIG_ERR, signal(SIGURG, SIG_DFL));
	TEST_ASSERT_EQUAL_INT(0, sigprocmask(SIG_SETMASK, &empty, NULL));
	TEST_ASSERT_EQUAL_HEX32(0, handler_haveSignal);
	TEST_ASSERT_NOT_EQUAL(SIG_ERR, signal(SIGURG, sighandler));
	TEST_ASSERT_EQUAL_HEX32(0, handler_haveSignal);
...
TEST(sigaction, unmask_changed_action_handler_to_default)
{
	...
	if (pid > 0) {
		int code;
		TEST_ASSERT_EQUAL_INT(pid, wait(&code));
		TEST_ASSERT_EQUAL_INT(true, WIFSIGNALED(code));
	}
	else {
		handler_haveSignal = 0u;
...
TEST(sigaction, handler_recursion_raise)
{
	...
	while (handler_countdown > 0) {
		TEST_ASSERT_EQUAL_INT(-1, sigsuspend(&empty));
	}
	TEST_ASSERT_EQUAL_INT(0, handler_countdown);
	set = handler_sigset;
	TEST_ASSERT_EQUAL_INT(1, sigismember(&set, SIGUSR1));
...
TEST(sigaction, sigaction_in_handler_handle)
{
	...
	TEST_ASSERT_EQUAL_INT(0, sigaction(SIGUSR2, &action, NULL));
...
TEST(sigaction, sigaction_in_handler_handle_reraise)
{
	...
	TEST_ASSERT_EQUAL_INT(0, sigaction(SIGUSR1, &action, NULL));
	TEST_ASSERT_EQUAL_INT(0, raise(SIGUSR1));
...
TEST(sigaction, sigaction_in_handler_nodefer_handle_reraise)
{
	...
	TEST_ASSERT_EQUAL_INT(0, sigaction(SIGUSR1, &actionNodefer, NULL));
	TEST_ASSERT_EQUAL_INT(0, raise(SIGUSR1));
	TEST_ASSERT_EQUAL_HEX32((1u << SIGUSR1), handler_haveSignal);
}
...
TEST(sigaction, sigaction_in_handler_ignore)
{
	...
	handler_sigaction.sa_flags = 0;
	handler_sigaction.sa_mask = empty;
	TEST_ASSERT_NOT_EQUAL(SIG_ERR, signal(SIGUSR1, &sighandlerAction));
	TEST_ASSERT_EQUAL_INT(0, raise(SIGUSR1));
	TEST_ASSERT_EQUAL_INT(0, raise(SIGUSR1));
	TEST_ASSERT_EQUAL_INT(0, raise(SIGUSR1));
...
TEST_GROUP_RUNNER(sigaction)
{
	...
	/*
	 * Unmasking with initial SIG_IGN is omitted, as:
