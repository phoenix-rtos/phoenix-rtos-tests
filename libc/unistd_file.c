...
/* simultanous write with two open descriptors */
TEST(unistd_file, file_write_reopened)
{
	...
	{
		/* write to the same file consequtively from two different file descriptors */
		TEST_ASSERT_EQUAL_INT(sizeof(LINE1) - 1, write(fd, LINE1, sizeof(LINE1) - 1));
...
/* test simultanous write() on duplicated file descriptor */
TEST(unistd_file, file_write_dup)
{
	...
	TEST_ASSERT_GREATER_OR_EQUAL(0, fdr);
	{
		/* write to the same file consequtively from descriptor and its copy */
		TEST_ASSERT_EQUAL_INT(sizeof(LINE1) - 1, write(fd, LINE1, sizeof(LINE1) - 1));
		TEST_ASSERT_EQUAL_INT(sizeof(LINE2) - 1, write(fd2, LINE2, sizeof(LINE2) - 1));
...
/* test write on readonly file descripor */
...
TEST(unistd_file, file_truncate_opened)
{
	...
	/* try to read from fd, shoud read only LINE2 */
...
{
	/* <posix incmpliance> ftruncate() able to succesfully set negative file size
...
/* ftruncate()-ing file with inalid file descriptors */
...
/* test if descriptor from dup() reffers to the same file */
...
/* test if descriptor from dup2() reffers to the same file */
...
/* dup2() on file descriptor that reffers to file opened by another descriptor */
