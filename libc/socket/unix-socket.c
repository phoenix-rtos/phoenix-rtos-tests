...
static void unix_select(int type)
{
	fd_set rfds;
	struct timeval tv;
	struct timespec ts[2];
	int rv, ms, fd, i;
	int bad_usecs[2] = { -1, 1000 * 000 /* 1000ms */ };
	...
	ms = MS_BETWEEN(ts[0], ts[1]);
	TEST_ASSERT(rv == 0);
	TEST_ASSERT_EQUAL_INT(0, FD_ISSET(fd, &rfds));
	TEST_ASSERT_LESS_THAN(350, ms);
	TEST_ASSERT_GREATER_THAN(290, ms);
	...
	for (i = 0; i < sizeof(bad_usecs) / sizeof(bad_usecs[0]); i++) {
		tv.tv_sec = 0;
		tv.tv_usec = bad_usecs[i];
		errno = 0;
		rv = select(-1, &rfds, NULL, NULL, &tv);
		TEST_ASSERT_EQUAL_INT(-1, rv);
		TEST_ASSERT_EQUAL_INT(EINVAL, errno);
	...
	tv.tv_usec = 1000;
	FD_ZERO(&rfds);
	FD_SET(fd, &rfds); /* TODO: soon-to-be bad fd */
	close(fd);
	errno = 0;
...
TEST(test_unix_socket, select)
{
	unix_select(SOCK_STREAM);
	unix_select(SOCK_DGRAM);
	unix_select(SOCK_SEQPACKET);
}
...
TEST_GROUP_RUNNER(test_unix_socket)
{
	...
	RUN_TEST_CASE(test_unix_socket, select);
}
