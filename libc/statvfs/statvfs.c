...
TEST(statvfs_statvfs, statvfs_f_flag_readonly)
{
	...
	memset(&buf, 0, sizeof(buf));
	TEST_ASSERT_EQUAL_INT(0, statvfs("/tmp", &buf));

	errno = 0;
	fd = open(STATVFS_TEST_FILE, O_CREAT | O_WRONLY, 0644);
	if (fd == -1) {
		/* creation failed so it must be a Read Only fs */
		TEST_ASSERT_EQUAL_INT(EROFS, errno);
		TEST_ASSERT_EQUAL_UINT(ST_RDONLY, (buf.f_flag & ST_RDONLY));
	}
	else {
		close(fd);
		test_common.fd = -1;
		TEST_ASSERT_EQUAL_UINT(0U, (buf.f_flag & ST_RDONLY));
	}
}
...
TEST(statvfs_statvfs, statvfs_enotdir_prefix_not_dir)
{
	...
	/* A component of the path prefix is a non-directory */
	errno = 0;
...
/* issue #1723: https://github.com/phoenix-rtos/phoenix-rtos-project/issues/1723 */
	...
	/* The path argument contains at least one non-slash char and ends with one */
	errno = 0;
	ret = statvfs(STATVFS_TEST_FILE "/", &buf);
...
TEST_GROUP_RUNNER(statvfs_statvfs)
{
	...
	RUN_TEST_CASE(statvfs_statvfs, statvfs_enotdir_trailing_slash_not_dir);
