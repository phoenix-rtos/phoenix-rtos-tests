...
/*
 * libphoenix does not define O_DIRECTORY nor O_NOFOLLOW, issue:
 * https://github.com/phoenix-rtos/phoenix-rtos-project/issues/1728
 * Fall back to a no-op value, so that the test cases below are always compiled
 * and reported - they are skipped by TEST_IGNORE until the issue is resolved.
 */
#ifndef O_DIRECTORY
...
{
	test_common.prevMask = umask(0);
	umask(test_common.prevMask);
...
#ifdef __phoenix__
	TEST_IGNORE_MESSAGE("#1728 issue");
#endif
...
TEST(fcntl_open, open_nonblock_fifo_rdonly)
{
	...
	TEST_ASSERT_EQUAL_INT_MESSAGE(0, ret, "TEST ERROR: mkfifo() failed");
