...
/* timeout 3s */
#define SPWPERF_RX_TIMEOUT 1000000 * 5
...
#define SPWPERF_TEST_HDRSZ 3u
#else
#define SPWPERF_TEST_HDRSZ 2u
#endif

#define SPWPERF_TEST_NPACKETS 128
...
static void spwperf_clientThread(void *arg)
{
...
#if TEST_SPW_LOOPBACK
	const uint8_t hdr[] = { 0x1, SPWPERF_S_ADDR, /* protocol ID */ 0x5 };
#else
	...
	for (int i = 0; i < SPWPERF_TEST_DATASZ; i++) {
		data[i] = i;
	}

	const size_t hdrSz = sizeof(hdr), dataSz = SPWPERF_TEST_DATASZ;
	...
	for (;;) {
		...
		mutexLock(spwperf_common.lock);
		condWait(spwperf_common.cond, spwperf_common.lock, 0);
		mutexUnlock(spwperf_common.lock);
	}
...
static void spwperf_serverThread(void *arg)
{
	...
	unsigned int firstDesc;
	spw_rxPacket_t packets[nPackets];

	gettimeofday(&start, NULL);
	int i = 0;
	firstDesc = spwperf_spwConfigureRx(spwperf_common.serverOid, nPackets);
	while ((time(NULL) - start.tv_sec) <= spwperf_common.ttime) {
		...
		firstDesc = spwperf_spwConfigureRx(spwperf_common.serverOid, nPackets);
		condSignal(spwperf_common.cond);
...
int main(int argc, char **argv)
{
	...
	LOG("TEST: nPackets: %d packets size: %d loopback %d", SPWPERF_TEST_NPACKETS, SPW_MAX_PACKET_LEN, TEST_SPW_LOOPBACK);

	int err;
	err = condCreate(&spwperf_common.cond);
	if (err != 0) {
		LOG_ERROR("Failed to create cond: %d", err);
		return -1;
	}

	err = mutexCreate(&spwperf_common.lock);
	if (err != 0) {
		LOG_ERROR("Failed to create lock: %d", err);
		return -1;
	}
